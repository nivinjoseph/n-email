...
class SendGridEmailService {
    constructor() {
        ...
        n_defensive_1.given(sendGridApiKey, "sendGridApiKey").ensureHasValue().ensureIsString();
        SendGrid.setApiKey(sendGridApiKey);
        const sendGridSenderEmail = n_config_1.ConfigurationManager.getConfig("sendGridSenderEmail");
        n_defensive_1.given(sendGridSenderEmail, "sendGridSenderEmail").ensureHasValue().ensureIsString();
        this._senderEmail = sendGridSenderEmail;
    }
    sendEmail(toEmail, subject, body, isBodyHtml = false) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            n_defensive_1.given(toEmail, "email").ensureHasValue().ensureIsString();
            n_defensive_1.given(subject, "subject").ensureHasValue().ensureIsString();
            n_defensive_1.given(body, "body").ensureHasValue().ensureIsString();
            n_defensive_1.given(isBodyHtml, "isBodyHtml").ensureHasValue().ensureIsBoolean();
            const message = {
                to: toEmail,
